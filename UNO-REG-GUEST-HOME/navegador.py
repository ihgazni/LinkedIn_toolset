...
class solicitud:
    ...
    def stepping_Resp(self,conn,keepalivetimeout,explicit=0):
        ...
        rbody = resp.read()
        secarino_1 = self.check_Resp_Headers(rheaders,'Connection','Keep-Alive')
        secarino_2 = self.check_Resp_Headers(rheaders,'Connection','keep-alive')
        if(secarino_1 == -1):
            rkeepalive = -1
        elif((secarino_1 == 0) & (secarino_2 == 0)):
            rkepalive = 0
